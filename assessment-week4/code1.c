...
int isValidOpcode(char *opcode) {
    ...
    if (strcmp(opcode, "START") == 0 || strcmp(opcode, "RESB") == 0 || 
        ...
        strcmp(opcode, "WORD") == 0 || strcmp(opcode, "END") == 0 || strcmp(opcode, "FIRST") == 0) {
        return -1;
    }
...
int main() {
    ...
    idk2 hex[10]; int h = 0;
    idk2 decimal[10]; int dec = 0;
    idk2 octal[10]; int oc = 0;
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (isValidOpcode(label) > 0) {
            strcpy(inst[i].label, ""); 
            strcpy(inst[i].opcode, label);
            if (opcode != NULL) {
                strcpy(inst[i].operand, opcode); 
            } else {
        ...
        if (format > 0) {
            ...
            if (inst[i].operand[0] == 'C') {
                ...
                strcpy(decimal[dec++].str, inst[i].operand);
            } else if (inst[i].operand[0] == 'X') {
                memoryCounter += (strlen(inst[i].operand) - 3) / 2;  
                strcpy(hex[h++].str, inst[i].operand); 
            } else if (inst[i].operand[0] == 'O') {
                memoryCounter += 2;  
                strcpy(octal[oc++].str, inst[i].operand);
            }
